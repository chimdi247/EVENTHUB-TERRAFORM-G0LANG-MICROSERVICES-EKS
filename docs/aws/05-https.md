...
EventHub is served at **https://vijaygiduthuri.in** with a real
...
flowchart TB
    ...
    S8["8 &middot; Traefik terminates TLS<br/>https://vijaygiduthuri.in"]
...
```bash
watch dig +short NS vijaygiduthuri.in @8.8.8.8
```
...
```bash
curl -s http://vijaygiduthuri.in/api/events | head -20
```
  ...
  └─► Certificate  eventhub-tls
        └─► CertificateRequest
              └─► Order            (ACME order with Let's Encrypt)
                    └─► Challenge  (DNS-01)
                          └─► TXT record _acme-challenge.vijaygiduthuri.in… in Route53
...
# Location: https://vijaygiduthuri.in/        <- no port. See the warning below.

curl -Ik https://vijaygiduthuri.in/
# HTTP/2 200

curl -v https://vijaygiduthuri.in/ 2>&1 | grep -i issuer
...
> `Location: https://vijaygiduthuri.in:8443/` — a port the load balancer does
...
> # final: 200 at https://vijaygiduthuri.in/
> ```
...
Open <https://vijaygiduthuri.in> — a padlock, no warning.
...
> resolvectl query vijaygiduthuri.in      # "Data from: synthetic" confirms /etc/hosts
...
>
> ```bash
> IP=$(dig +short vijaygiduthuri.in @8.8.8.8 | head -1)
> BASE_URL=https://vijaygiduthuri.in RESOLVE_IP=$IP ./scripts/smoke-test.sh
> ```
...
| Certificate stuck `False`, challenge `pending` | Public DNS cannot see the zone yet | `dig +short NS vijaygiduthuri.in @8.8.8.8` — wait for delegation. |
...
EventHub is live at **https://vijaygiduthuri.in** — five Go
