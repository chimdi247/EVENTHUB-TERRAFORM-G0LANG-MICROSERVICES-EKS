...
```mermaid
graph TB
    User([User]) -->|HTTPS| R53[Route53<br/>vijaygiduthuri.in]
...
| Hostname | `vijaygiduthuri.in (apex)` | `eventhub-stage.…` | `eventhub.…` |
...
- A registered domain. This repo assumes `vijaygiduthuri.in` on GoDaddy.
...
Set those four nameservers at GoDaddy: **My Products → vijaygiduthuri.in → DNS →
Nameservers → Change → I'll use my own nameservers**. No trailing dot.

```bash
dig +short NS vijaygiduthuri.in @8.8.8.8
```
